...
const STEP_LABELS = ["Arrangement", "Kamerselectie", "Bevestiging"];

export function Breadcrumb({ currentStep, onNavigate, title }: BreadcrumbProps) {
  return (
    <div className="mb-8">
      ...
      <div className="text-sm text-gray-500 mb-2">
        {STEP_LABELS.map((label, idx) => {
          ...
          const clickableClass = isPast && onNavigate
            ? "cursor-pointer hover:text-[#2C4A3C]"
            : "";
          ...
          return (
            <React.Fragment key={label}>
              <span onClick={handleClick} className={`${textClass} ${clickableClass}`}>
                {label}
              </span>
